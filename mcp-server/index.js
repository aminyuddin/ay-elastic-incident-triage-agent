/**
 * Incident Portal MCP Server — Streamable HTTP.
 * Exposes the same tools as the Cloud Function (functions/mcpApp.js):
 *   create_incident, list_incidents, close_incident, reopen_incident
 * Writes/reads Firebase Firestore (same project as the Vite portal). For use with Elastic Agent Builder MCP connector.
...
function requireAuth(req, res, next) {
  if (!isSecretConfigured()) {
    res.status(503).set('Content-Type', 'application/json').json({
      jsonrpc: '2.0',
      error: { code: -32000, message: 'MCP auth not configured. Set MCP_AUTH_SECRET and send Authorization: Bearer <secret> or X-API-Key: <secret>.' },
      id: null,
    });
    return;
  }
...
// Create MCP server with tools (per mcp-api.md)
function createServer() {
  const server = new McpServer(
    {
      name: 'incident-portal-mcp',
      version: '1.0.0',
    },
    {
  ...
  // create_incident — writes to Firestore `incidents` collection
  server.registerTool(
    'create_incident',
    {
      description:
    ...
    async (args) => {
      const createdBy = process.env.MCP_CREATED_BY || 'elastic-agent';
      const payload = {
      ...
      const incident = await createIncident(payload, createdBy);
  ...
  // list_incidents — reads from Firestore, ordered by created_at DESC; optional status filter
  server.registerTool(
    'list_incidents',
    {
      description:
  ...
  // close_incident — set incident status to CLOSED by Firestore document id
  server.registerTool(
    'close_incident',
    {
      description:
  ...
  // reopen_incident — set incident status to OPEN, clear closed_at
  server.registerTool(
    'reopen_incident',
    {
      description:
...
// Express app: POST /mcp, GET /mcp, DELETE /mcp (Streamable HTTP + SSE)
const app = express();
app.use(express.json());

// Protect MCP routes with header auth (skip /health)
...
// Health check for deploy/load balancers
app.get('/health', (req, res) => {
  res.json({ ok: true, service: 'incident-portal-mcp' });
});

app.listen(PORT, '0.0.0.0', () => {
  console.log(`Incident Portal MCP server listening on http://0.0.0.0:${PORT}`);
  console.log(`  MCP endpoint: http://localhost:${PORT}/mcp`);
  console.log(`  Kibana connector Server URL: http://<host>:${PORT}/mcp (or https://... if behind TLS)`);
});
